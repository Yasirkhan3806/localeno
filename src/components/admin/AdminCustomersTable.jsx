...
const AdminCustomersTable = ({
  ...
  useEffect(() => {
    const filteredCustomers = userData.filter(
      (s) => s?.userData?.accountType === "customer"
    );
    const oneMonthAgo = new Date();
    oneMonthAgo.setMonth(oneMonthAgo.getMonth() - 1);

    const newSellers = filteredCustomers.filter((s) => {
      const createdDate = new Date(s?.userData?.accountCreated);
      return createdDate > oneMonthAgo;
    });

    const newSellersCount = newSellers.length;
    setNewCustomers(newSellersCount);
  ...
  const KPI = [
    {
      icon: (
        <svg
          className="w-7 h-7 text-blue-700 bg-blue-100 rounded-md p-1"
          fill="none"
          viewBox="0 0 24 24"
        >
    ...
    },
    {
      icon: (
        <svg
          className="w-7 h-7 text-green-700 bg-green-100 rounded-md p-1"
          fill="none"
          viewBox="0 0 24 24"
        >
      ...
      value: customers
        .filter((c) => c?.userData?.status === "active")
  ...
  //   c =>
  //     c?.userData?.firstName.toLowerCase().includes(search.toLowerCase()) ||
  //     c?.userData?.email.toLowerCase().includes(search.toLowerCase()) ||
  //     c.id.toLowerCase().includes(search.toLowerCase())
  // );

  const handleMoreActions = async (customer, custId) => {
    const action = window.confirm("Delete Customer ?");

    if (action) {
      try {
        await deleteDoc(doc(db, "userData", custId));
  ...
  const handleViewCustomer = (customer) => {
    setViewCustomer(customer?.userData);
  };
          ...
          {customers.filter(
            (s) =>
              ...
              s?.userData?.lastName
                ?.toLowerCase()
                .includes(search.toLowerCase()) ||
              s?.userData?.email?.toLowerCase().includes(search.toLowerCase())
          ).length === 0 ? (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between">
                          ...
                          >
                            ...
                            <button
                              ...
                              title="More actions"
                              onClick={() =>
                                handleMoreActions(customer, customer.id)
                              }
                            >
                        ...
                        <div className="flex items-center justify-between mt-2">
                          {statusBadge(customer?.userData?.status)}
